...
export const forgotPassword = createAsyncThunk('auth/forgotPassword', async (u, t) => {
  try {
    return await authService.forgotPassword(u);
  } catch (e) {
    const message = (e.response && e.response.data && e.response.data.message) || e.message || e.toString();
    return t.rejectWithValue(message);
  }
...
export const resetPassword = createAsyncThunk('auth/resetPassword', async (u, t) => {
  try {
    return await authService.resetPassword(u);
  } catch (e) {
    const message = (e.response && e.response.data && e.response.data.message) || e.message || e.toString();
    return t.rejectWithValue(message);
  }
...
const authSlice = createSlice({
  ...
  extraReducers: (b) => {
    b.addCase(register.pending, (s) => { s.isLoading = true; })
     ...
     .addCase(login.fulfilled, (s, a) => { s.isLoading = false; s.isSuccess = true; s.user = a.payload; })
     ...
     .addCase(resendOtp.fulfilled, (s, a) => { s.isLoading = false; s.isSuccess = true; s.message = a.payload.message; })
      ...
      .addCase(forgotPassword.fulfilled, (s, a) => { s.isLoading = false; s.isSuccess = true; s.message = a.payload.message; })
      .addCase(forgotPassword.rejected, (s, a) => { s.isLoading = false; s.isError = true; s.message = a.payload; })
      .addCase(resetPassword.pending, (s) => { s.isLoading = true; })
      .addCase(resetPassword.fulfilled, (s, a) => { s.isLoading = false; s.isSuccess = true; s.message = a.payload.message; })
      .addCase(resetPassword.rejected, (s, a) => { s.isLoading = false; s.isError = true; s.message = a.payload; })
